...
class FocalHeatmapLoss(KeypointMSELoss):
    ...
    def forward(self,
        ...
        assert not torch.isnan(target).any(), "NaN detected in target"
        _mask = self._get_mask(target, target_weights, mask)
        ...
        eps = 1e-3
        output = torch.clamp(output.float(), eps, 1.0 - eps)
        pos_loss = (torch.log(output) * torch.pow(1 - output,
                                                 self.alpha) * pos_inds).bfloat16()
        neg_loss = (torch.log(1 - output) * torch.pow(
            output, self.alpha) * neg_weights * neg_inds).bfloat16()
        ...
        # Check for NaN or inf values in the loss and print debug information
        if torch.isnan(loss) or torch.isinf(loss):
        # if True:
            print("Detected NaN or inf in loss calculation!")
            print(f"num_pos: {num_pos}")
            print(f"pos_loss sum: {pos_loss.sum().item()}")
            print(f"neg_loss sum: {neg_loss.sum().item()}")
            print(f"pos_inds sum: {pos_inds.sum().item()}")
            print(f"neg_inds sum: {neg_inds.sum().item()}")
            print(f"neg_weights sum: {neg_weights.sum().item()}")
